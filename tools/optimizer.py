...
Pricing sourced live from Yahoo Finance (yfinance).
...
# ─────────────────────────────────────────────────────────────────────────────
# Try to import yfinance; guide user if missing
# ─────────────────────────────────────────────────────────────────────────────
try:
...
def fetch_market_data(tickers: List[str], risk_free_rate: float = 0.0525) -> dict[str, MarketSnapshot]:
    """
    Pull live spot prices and estimate ATM IV from the nearest-expiry
    ...
    for ticker in tickers:
        ...
        try:
            ...
            # ── Estimate IV from the option chain closest to 30 DTE ──────────
            exps = tkr.options                         # available expiry dates
            if not exps:
                raise ValueError("No options chain available")

            today = date.today()
            # pick expiry nearest to 30 DTE
            best_exp = min(
                exps,
                key=lambda e: abs((datetime.strptime(e, "%Y-%m-%d").date() - today).days - 30)
            )
            chain = tkr.option_chain(best_exp)

            # ATM IV: average of nearest put + call to current spot
            for df in (chain.calls, chain.puts):
                df = df[df["impliedVolatility"] > 0].copy()
                df["dist"] = (df["strike"] - spot).abs()

            calls_atm = chain.calls.assign(dist=(chain.calls.strike - spot).abs())
            puts_atm  = chain.puts.assign( dist=(chain.puts.strike  - spot).abs())

            call_iv = float(calls_atm.sort_values("dist").iloc[0]["impliedVolatility"])
            put_iv  = float(puts_atm.sort_values( "dist").iloc[0]["impliedVolatility"])
...
STRIKE_MONEYNESS = [-0.12, -0.10, -0.07, -0.05, -0.03, 0.00,
                     0.03,  0.05,  0.07,  0.10,  0.12]
EXPIRY_DTE       = [7, 14, 21, 30, 45, 60]
